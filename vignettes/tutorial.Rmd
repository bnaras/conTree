...
contrast tree and distribution boosting procedures described in Friedman
(2020). A brief summary of the methodology is provided. This is followed
by demonstrations of the application of some of the package features on
data examples. A more detailed description of all procedures is included
with the full package documentation. Some familiarity with Friedman
(2020) and the statistical package R is assumed. R can be downloaded
...
$$\hat{z}(\mathbf{x})=z(\mathbf{x})+\sum_{k=1}^{K}\delta_{m_{k}(\mathbf{x})}^{(k)}\text{.}$$
...
$$p_{g_{\mathbf{x}}}(g_{\mathbf{x}}(z\,)\mathbf{\,|\,x})=p_{y}(y\,|\,\mathbf{x})\text{.}$$ Thus, starting with $z$ values sampled from a
...
contrast trees $\{R_{m}^{(k)}\}_{m=1\text{ }k=1}^{M\text{ \ \ \ \ }K}$.
...
$$\hat{g}_{\mathbf{x}}(z\,)=g_{m_{K}(\mathbf{x})}^{(K)}(g_{m_{K-1}(\mathbf{x})}^{(K-1)}(g_{m_{K-2}(\mathbf{x})}^{(K-2)}\cdot\cdot\cdot g_{m_{1}(\mathbf{x})}^{(1)}(z)))\text{.} $$ That is, the transformed
...
`$\Rightarrow$  ordinary one-sample tree contrasting $y$ and
$z$. `mode = "twosamp"` $\Rightarrow$ two-sample tree contrasting
outcomes $y$ for samples of size $n_{1}$ and $n_{2}$ respectively. In
this mode $n=n_{1}+n_{2}$. `x`  contains the $n\times p$ training
predictor variable data matrix or data frame of the pooled sample,
 `y` contains the corresponding pooled outcome values and `z` is a
vector of length $n$ specifying sample identity. `z[i] < 0`
...
discrepancy
$$d_{m}=\frac{1}{N_{m}}\sum_{\mathbf{x}_{i}\in R_{m}}|\,y_{i}-z_{i}\,|\text{.}$$
...
R_{m}}I(y_{i}<z_{i})\right\vert$$ in the region. For this type an
...
}{\sqrt{i\cdot(2N_{m}-i)}}$$ where $t_{(i)}$ is the $i$th value of $t$
...
$$d_{m}=\frac{1}{N_{m}}\sum_{\mathbf{x}_{i}\in R_{m}}C(y_{i},z_{i})$$
...
The parameters `x, y, z` and `tree` are the same as in `nodesum()`
above.  `nodes` is a vector of tree terminal node identifiers
specifying the regions to be displayed . The default is all terminal
nodes except for `type = "dist"` or `type = "diff"` for which the
default is the nine highest discrepancy regions. `pts = TRUE/FALSE`
will show points as circles/points (`type = ’dist’` only).
...
`x` is an input predictor data matrix or data frame with same variables
and structure input to `contrast()`. Rows are observations and columns are
variables. Must be a numeric matrix or a data frame. `tree` is a tree
model object output from *contrast*. The output of getnodes `nx` is a
vector of tree terminal node identifiers (numbers) corresponding to each
...
```{r, eval = FALSE}
out = lofcurve(x, y, z, tree, doplot = TRUE)`
```
...
`x` and `z` are the $x$ and $z$-values for new data of the same type
input to `modtrast()`. `model` is a model object output from `modtrast()`.
`num` is the number of trees used to compute model values. Default is
the number contained in model as produced by `modtrast()`. The output
`ypred` is a vector of predicted values for new data by the model.
...
The input `x` represents the components of a *single* point $\mathbf{x}$
in predictor variable $\mathbf{x}$-space. `z` is a vector of values to
be transformed. These are usually the quantiles of the prespecified
distribution $p_{z}(z\,|\,\mathbf{x}_{i}\mathbf{)}$ at $\mathbf{x}$.
`model` is a model object output from `modtrast()`. `num` is number of
trees used to compute model values. Default is the number contained in
model as produced by *modtrast*. The output `yhat` contains the
corresponding transformed $z$ - values.
...
analysis presented in Section 6.2 of Friedman (2020) using the census
...
```{r, fig = TRUE, fig.caption = "Figure 1"}
...
Interpretation of this output is described in Section
[\[treesum\]](#treesum){reference-type="ref" reference="treesum"}.
...
tree represented in Fig. [\[fig1\]](#fig1){reference-type="ref"
reference="fig1"}. The commands
...
plot the test data GBP predictions against those of GBL for the highest
discrepancy region 7. The red line represents equality. The result shown
in Fig. [\[fig3\]](#fig3){reference-type="ref" reference="fig3"}
indicates that the gradient boosting probabilities based on log-odds
estimates in this region are considerably larger but proportional to
those obtained by direct probability estimation using least-squares. One
also sees the effect of truncating the out of range estimates produced
by the latter. In spite of this the GBP probability estimates are seen
in Fig. [\[fig2\]](#fig2){reference-type="ref" reference="fig2"} (below)
to be from three to four times more accurate than those of GBL
...
u = lofcurve(xt[dxt, ], yt[dxt], rft[dxt, 2], tree, doplot = TRUE)
...
u = lofcurve(xt[dxt, ], yt[dxt], gbpt[dxt], tree, doplot = TRUE)
lines(u$x, u$y, col = 'red')
```

The result is shown in Fig. [\[fig2\]](#fig2){reference-type="ref"
reference="fig2"}.
...
The result is shown in Fig.[\[fig4\]](#fig4){reference-type="ref"
reference="fig4"}. Note that results at every tenth iteration are shown.
...
producing Fig. [\[fig5\]](#fig5){reference-type="ref" reference="fig5"}.

Comparing with Fig. [\[fig1\]](#fig1){reference-type="ref"
reference="fig1"} one sees that contrast boosting the GBL model appears
to have substantially reduced the shrinking of the of the extreme
probabilities estimates thereby improving its conditional probability
estimation accuracy.
...
produces the marginal age distribution as seen in Fig
 [\[fig6\]](#fig6){reference-type="ref" reference="fig6"}.
...
We next contrast the distribution of $y\,|\,\mathbf{x}$ with that of the
no information hypothesis $p_{y}(y\,|\,\mathbf{x})=\hat{p}_{y}(y)$ where
$\hat
{p}_{y}(y)$ is the empirical marginal $y$ - distribution independent of
$\mathbf{x}$ as shown in Fig. [\[fig6\]](#fig6){reference-type="ref"
reference="fig6"}. First create independent marginal distribution of $y$
as the contrasting distribution
...
shown in Fig. [\[fig7\]](#fig7){reference-type="ref" reference="fig7"}.

Each frame in Fig. [\[fig7\]](#fig7){reference-type="ref"
reference="fig7"} shows a QQ-plot of the distribution of age $y$
versus that of its $\mathbf{x}$ independent marginal counterpart (Fig.
[\[fig6\]](#fig6){reference-type="ref" reference="fig6"}) in each of
...
The result is shown in Fig. [\[fig8\]](#fig8){reference-type="ref"
reference="fig8"}.
...
shown is Fig. [\[fig9\]](#fig9){reference-type="ref" reference="fig9"}.
...
test data. Results are shown in
Fig.[\[fig95\]](#fig95){reference-type="ref" reference="fig95"}.

As seen the results are not perfect but somewhat better than that for
the residual bootstrap distribution shown in Fig.
[\[fig8\]](#fig8){reference-type="ref" reference="fig8"}. This is
verified by the corresponding lack-of-fit contrast curves shown in Fig.
[\[fig10\]](#fig10){reference-type="ref" reference="fig10"} as produced
by the commands

```{r}
lofcurve(xage[dt, ], yage[dt], zage[dt], treezage)
u = lofcurve(xage[dt, ], yage[dt], rbage[dt], treerbage, doplot = FALSE)
lines(u$x, u$y, col = 'blue')
u = lofcurve(xage[dt, ], yage[dt], hrbage, treehrbage, doplot = FALSE)
lines(u$x, u$y, col = 'red')
```
...
```
as shown in Fig. [\[fig11\]](#fig11){reference-type="ref"
reference="fig11"}. The red points shown at the bottom of each plot
...
```
as seen in Fig.
[\[fig12\]](#fig12){reference-type="ref" reference="fig12"}.
...
displays the distribution of the input propensity probability scores in
Fig. [\[fig13\]](#fig13){reference-type="ref" reference="fig13"}. One
sees that there are moderate differences between the $\mathbf{x}$ -
distributions of the two samples at least globally.
...
produces the graphical summary shown in Fig.
[\[fig14\]](#fig14){reference-type="ref" reference="fig14"}. The
...
this procedure. Fig. [\[fig17\]](#fig17){reference-type="ref"
reference="fig17"} shows the histogram of these null discrepancies along
...
As described in Section [\[treesum\]](#treesum){reference-type="ref"
reference="treesum"} the output for each terminal node shows the
...
We next contrast the conditional distributions $y\,|\,\mathbf{x}$ of the
two samples. Figure [\[fig15\]](#fig15){reference-type="ref"
reference="fig15"} shows a QQ-plot between the respective $y$ global
distributions on the test data obtained by the commands
...
as shown in Fig. [\[fig16\]](#fig16){reference-type="ref"
reference="fig16"}. The tree has uncovered a few regions where there are
moderate differences between the two distributions.
